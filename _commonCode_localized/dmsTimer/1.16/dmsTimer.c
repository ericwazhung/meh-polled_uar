...
#if(_DMS_OCR_VAL_ <= 255)
uint8_t dmsNumUpdates = 100;
#else
uint16_t dmsNumUpdates = 100;
#endif
...
//Usually this'll be Timer0 Compare-Match A, but can be overridden
SIGNAL(DMS_AVRTIMER_SIG)
{
   dmsCount++;
}
...
// #endif
   DMS_AVRTIMER_OCR = _DMS_OCR_VAL_;

   timer_setWGM(DMS_AVRTIMER_NUM, WGM_CLR_ON_COMPARE);
   timer_selectDivisor(DMS_AVRTIMER_NUM, _DMS_CLKDIV_);
   timer_compareMatchIntEnable(DMS_AVRTIMER_NUM, DMS_AVRTIMER_OUT_CHANNEL);
 ...
 * /home/meh/_commonCode/polled_uar/0.51-gitHubbing/testMega328p/_commonCode_localized/dmsTimer/1.16/dmsTimer.c
 *
