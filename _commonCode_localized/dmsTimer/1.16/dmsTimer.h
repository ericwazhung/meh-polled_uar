...
//1.16 adjusting to timerCommon 1.23 for gitHubbing of polled_uar
//1.15-2 these IsItTime functions are named badly...
//       left for backwards-compatibility, but they should be:
//       dms_isItTime()
//1.15-1 cleaning up some error-messages for PWM161...
//       They were just used for testing
//       heartbeat/1.50/testPWM161+button+DMS's transition...
//1.15 adding defautls for DMS_AVRTIMER_'s with PWM161...
//1.14-2 adding IsItTime...V2 for allowCumulation, which should be
//       smaller-codsize
//1.14-1 a/o heartbeat1.50...
//       defaults not working with atTiny861...
//       #include <avr/io.h> needed in header-file...
//1.14 continuation of 1.13-6
//     trying to generalize timer-usage
//     apparently the at90pwm161 doesn't have timer0 at all, just timer1
//     THIS VERSION IS CURRENTLY UNTESTED
//       pending necessary changes to timerCommon
//       a/o heartbeat/testPWM161+button+DMS
//     Now at timerCommon 1.22
//     TODO: Instead of using xyt, use hfm??? Would be MUCH smaller... no?
...
//Necessary here for SIG_OUTPUT_COMPARE!, etc...
 #include <avr/io.h>
#endif
...
#if(defined(__AVR_ARCH__) &&           \
    (!defined(_DMS_EXTERNALUPDATE_) || !_DMS_EXTERNALUPDATE_))


// SEE:
// http://www.nongnu.org/avr-libc/user-manual/group__avr__interrupts.html
//
// Regarding poisoned signals.
//
//These defaults are not really overrideable for the PWM161, since it only
//has one timer and it happens to be Timer1, and it only has CLKDIV1
//So, there's basically no other option for these
#if(defined(__AVR_AT90PWM161__))
  #define DMS_AVRTIMER_SIG TIMER1_CAPT_vect
  #define DMS_AVRTIMER_OCR ICR1
  #define DMS_AVRTIMER_NUM 1
  #define DMS_AVRTIMER_TCNT   TCNT1
  #define DMS_AVRTIMER_MAX    0xffff
  #define _DMS_CLKDIV_  CLKDIV1
#endif


//DMS_AVRTIMER_SIG is the timer output-compare interrupt signal handler...
// if it's not overridden, then it uses Timer0 compare-match A
#if(!defined(DMS_AVRTIMER_SIG))
 #if defined(TIMER0_COMP_A_vect)
  #define DMS_AVRTIMER_SIG          TIMER0_COMP_A_vect
 #elif defined(TIMER0_COMPA_vect)
  #define DMS_AVRTIMER_SIG           TIMER0_COMPA_vect
// This has likely been "poisoned" as well, see below
// But it should be OK to remain for now (for the "poisoned" warning)
// if I forgot something... because it comes *after* the replacements
 #elif defined(SIG_OUTPUT_COMPARE0A)
  #define DMS_AVRTIMER_SIG           SIG_OUTPUT_COMPARE0A
// #elif defined(SIG_OUTPUT_COMPARE0)
//  #define DMS_AVRTIMER_SIG           SIG_OUTPUT_COMPARE0
// The above has been "poisoned" and replaced by:
 #elif(defined(TIMER0_COMP_vect))
  #define DMS_AVRTIMER_SIG           TIMER0_COMP_vect
 #else
  #error "This MCU's Timer0 interrupt is not supported (or nonexistent)"
  #error " You can set another signal manually with e.g. in makefile:"
  #error "  CFLAGS+=-D'DMS_AVRTIMER_SIG=TIMER1_COMPA_vect'"
  #error " Or look into _DMS_EXTERNALUPDATE_"
 #endif
#endif

#if(!defined(DMS_AVRTIMER_OCR))
 #if defined(OCR0)
  #define DMS_AVRTIMER_OCR           OCR0
 #elif defined(OCR0A)
  #define DMS_AVRTIMER_OCR           OCR0A
 #else
  #error "DMS doesn't yet support this processor with dedicated Timer0."
  #error " You can set DMS_AVRTIMER_OCR manually, or look into DMS_EXTERNAL_UPDATE"
 #endif
#endif

#if(!defined(DMS_AVRTIMER_OUT_CHANNEL))
 #define DMS_AVRTIMER_OUT_CHANNEL    OUT_CHANNELA
#endif

//Unless overridden, use Timer0
#if(!defined(DMS_AVRTIMER_NUM))
 #define DMS_AVRTIMER_NUM            0
#endif


#if(!defined(DMS_AVRTIMER_TCNT))
 #if defined(TCNT0)
  #define DMS_AVRTIMER_TCNT          TCNT0
 #elif defined(TCNT0L)
  //This isn't particularly all-encompassing...
  // The Tiny861 uses TCNT0L alone in 8-bit mode (which should do)
  #define DMS_AVRTIMER_TCNT          TCNT0L
 #else
  #error "TCNT0 is not available... untested MCU!"
  #error "You can set DMS_AVRTIMER_TCNT manually,"
  #error " look into DMS_EXTERNALUPDATE,"
  #error " Or, since TCNT is only used for dmsFrac, disable it with DMS_FRAC_UNUSED"
 #endif
#endif


#if(!defined(DMS_AVRTIMER_MAX))
   //Assuming an 8-bit timer...
   #define DMS_AVRTIMER_MAX          255
#endif
...
// since it's the least likely to be defined on other architectures
 ...
 #if (_DMS_OCR_VAL_UNLIMITED_ <= DMS_AVRTIMER_MAX)
...
//         (if FALSE)
static __inline__
uint8_t dmsIsItTime6secV2(dms6sec_t *startTime, dms6sec_t deltaTime,
...
static __inline__
uint8_t dmsIsItTimeV2(dms4day_t *startTime, dms4day_t deltaTime, 
                           uint8_t dontAllowCumulation)
{
...
#define dmsIsItTime(a,b) dmsIsItTimeV2((a),(b),TRUE)
#define dmsIsItTime6sec(a,b) dmsIsItTime6secV2((a),(b),TRUE)


//These should be the names:
#define dms_isItTime(a,b)     dmsIsItTimeV2((a),(b),TRUE)
#define dms_isItTime6sec(a,b) dmsIsItTime6secV2((a),(b),TRUE)
 ...
 * /home/meh/_commonCode/polled_uar/0.51-gitHubbing/testMega328p/_commonCode_localized/dmsTimer/1.16/dmsTimer.h
 *
