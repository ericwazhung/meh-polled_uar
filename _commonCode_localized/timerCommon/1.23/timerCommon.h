...
//1.23-1 adding it to timerCommon.mk...
//1.23 _ERRORHANDLING_HEADER_ is now required, so we don't try to 'make
//     localize' without it. a/o gitHubbing heartbeat
//     TODone: (Mightn't it make more sense to add that in timerCommon.mk?)
//1.22 a/o at90pwm161 (a/o LCDdirectLVDS67, a/o heartbeat/testPWM161+..DMS)
//     at90pwm161 has only one timer, and it's funky.
//     A) It's deemed "Timer1"
//     B) It's *really* simplistic
//        a) It only has two WGM modes, overflow and compare-match
//        b) It only has a small handful of clock sources and divisors
//     TODO: A lot of conditional stuff, and #defines rely on the idea that
//           the timers are very similar amongst all devices
//           EXCEPT I keep finding exceptions.
...
#include <avr/interrupt.h>
 ...
 #error "timerCommon v1.23 explicitly requires _ERRORHANDLING_HEADER_"
 // This, so that we don't try to 'make localize' without it.
...
//Clock Select Bit Description to be used in the Register containing 
// the Clock Select Bits
//A/B in TCCRn does NOT indicate CHANNEL A/B, but Control Register A/B 
...
//at90pwm161:
// Timer0 ******DNE*****
// Timer1 16bit ******* Non-standard in every way *******
...
//Other divisors may be available and are somewhat burried below...
 ...
 // This isn't true for some chips!
 // e.g. tinyx61, listed above?
 #define IS_CSTYP(tn) (((tn) == 0) || ((tn) == 1))
#else //__AVR_AT90PWM161__
 #define IS_CSTYP(tn) (0)
 // the pwm161 doesn't have output pins for Timer1
 #define TIMER_SETOUTPUTMODES_UNUSED TRUE
#endif
...
// *PWM161's timer only has WGM_NORMAL and WGM_CLR_ON_COMPARE
//  (so matches TinyX61's Timer0?)
 ...
 * /home/meh/_commonCode/polled_uar/0.51-gitHubbing/testMega328p/_commonCode_localized/timerCommon/1.23/timerCommon.h
 *
