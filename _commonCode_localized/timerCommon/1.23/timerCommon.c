...
//This handles the PWM161, as well as others
#if defined(TCCR1B)
...
// DIV1 does correspond to PWM161
...
uint8_t timer_selectDivisor(uint8_t timerNum, uint8_t clockDiv)
{
   ...
   return 0;
   //The remaining code should get optimized-out
   // right, it would... but it's still gotta be looked-at
   // which isn't possible since a lot of the macros aren't defined.
 #else
   uint8_t csbits;
  #if defined(CSOBS)
 ...
 * /home/meh/_commonCode/polled_uar/0.51-gitHubbing/testMega328p/_commonCode_localized/timerCommon/1.23/timerCommon.c
 *
